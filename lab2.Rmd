...
## Format map (remove monomorphic)
```{r remove-mono-from-map}
maf <- col.summary(hapmap$genotypes)$MAF
hapmap$map <- hapmap$map %>%
  mutate(MAF = maf) %>% filter(MAF > 0)
# nrow(hapmap$map)  1283751 left
```
...
#uncor geno is weird-- only 106 individuals
...
## remove monomorphic from geno
```{r format geno data}
geno <- as(uncor_geno, "numeric")
uncor_geno <- which(maf != 0) 
monomorphic <- which(maf == 0) 
geno.clean <- geno[,-monomorphic]
...
## Run LM - uncorr, null model

## Run LM - uncorr, SNP-related model



# Run LMM model

## Run LM - corr, null model

## Run LM - corr, SNP-related model
